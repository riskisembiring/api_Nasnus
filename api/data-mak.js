...
export const deleteDataMakHandler = async (req, res) => {
  const { id } = req.body;

  if (!id) {
    return res.status(400).json({ message: 'ID data MAK diperlukan.' });
  }

  try {
    const docRef = doc(db, 'dataMak', id);
    await deleteDoc(docRef);

    return res.status(200).json({
      message: 'Data MAK berhasil dihapus',
      id,
    });
  } catch (error) {
    console.error('Error deleting data MAK:', error);
    return res.status(500).json({
      message: 'Terjadi kesalahan pada server',
      error: error.message,
    });
